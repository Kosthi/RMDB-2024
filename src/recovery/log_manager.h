...
public:
    ...
    void deserialize(const char *src) override {
        ...
        update_value_.Deserialize(src + OFFSET_LOG_DATA + sizeof(int) + old_value_.size, old_value_.size);
        int offset = OFFSET_LOG_DATA + sizeof(int) + old_value_.size + update_value_.size;
        rid_ = *reinterpret_cast<const Rid *>(src + offset);
        offset += sizeof(Rid);
        table_name_size_ = *reinterpret_cast<const size_t *>(src + offset);
        offset += sizeof(size_t);
